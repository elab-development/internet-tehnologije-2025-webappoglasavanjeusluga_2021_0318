...
export default function HomePageContent({ categories }: Props) {
    ...
    const [allServices, setAllServices] = useState<FullServiceDto[]>([]);
    const [services, setServices] = useState<FullServiceDto[]>([]);
    ...
    if (categoryId !== null)
        data = data.filter(d => d.category.id === categoryId);
